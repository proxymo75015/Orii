...
/**
 * Processor pour les notifications WhatsApp en japonais.
 *
 * Extrait l'expéditeur du ticker en recherchant le marqueur "からのメッセージ" (signifiant "message de").
 * Le traitement du message suit une logique similaire aux autres processeurs.
 */
class JapaneseProcessor(
    ...
    override fun isGroupChat(): Boolean {
        val textVal = text ?: throw NullPointerException("text is null")
        // Si le texte contient "グループ" (group), on considère qu'il s'agit d'un chat de groupe.
        if (textVal.contains("グループ")) return true
